...
## Результаты Security-сканирования (SAST)

### Общая статистика SpotBugs

| Метрика | Значение |
|---------|----------|
| Всего классов проанализировано | 18 |
| Всего найдено багов | 3 |
| Высокий приоритет (High) | 1 |
| Средний приоритет (Medium) | 2 |

---

### Найденные нарушения

#### Нарушение #1: Reliance on default encoding (HIGH)

**Местоположение:** `JwtAuthService.java`, строка 27

**Категория:** I18N (Internationalization)

**Приоритет:** High (1)

**Тип:** `DM_DEFAULT_ENCODING`

**Rank:** 19

**Описание проблемы:**

Метод `String.getBytes()` вызывается без указания кодировки, что приводит к использованию системной кодировки по умолчанию. Это вызывает разное поведение приложения на разных платформах.

**Влияние:** JWT токен, сгенерированный на одной ОС, может не валидироваться на другой.

**Проблемный код:**

```java
secret.getBytes()  // Без указания кодировки
```
...
| # | Приоритет | Файл | Строка | Тип | Описание |
|---|-----------|------|--------|-----|----------|
| 1 | High | `JwtAuthService.java` | 27 | `DM_DEFAULT_ENCODING` | `getBytes()` без кодировки |
| 2 | Medium | `CityResponseDTO.java` | 9 | `EI_EXPOSE_REP` | Возврат мутабельного списка |
| 3 | Medium | `CityResponseDTO.java` | 9 | `EI_EXPOSE_REP2` | Сохранение ссылки на внешний список |
